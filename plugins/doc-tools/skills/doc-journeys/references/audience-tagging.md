...
- **Journey-level documentation** (the end-to-end page for a journey, whatever its declared spine, aimed at end-users — the people who consume the product through its supported journeys) → `end-user`.

Cross-product journey pages are journey-level and therefore always `end-user`, regardless of spine. A wayfinding page written as `explanation` is still for the consumer of the platform, not its maintainer — the Diátaxis type describes the register, not the audience, and the two are independent here.
