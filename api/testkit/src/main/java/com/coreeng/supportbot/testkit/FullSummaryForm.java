...
public class FullSummaryForm {
    ...
    public static class Receiver implements StubWithResult.Receiver<FullSummaryForm> {
        ...
        public FullSummaryForm assertAndExtractResult(ServeEvent servedStub) throws Exception {
            ...
            JsonNode view = OBJECT_MAPPER.readTree(rawView);
        ...
        private String buildExpectedBlocksJson() {
            String teamInitialOption = ticket.team() != null ? String.format("""
                        "initial_option": {
            ...
            String tagsInitialOptions =
                    !ticket.tags().isEmpty() ? String.format("""
                        "initial_options": [%s],
            ...
            boolean isAssignmentEnabled =
                    !config.mocks().slack().supportMembers().isEmpty();
            ...
            return String.format(
                    ...
                    headerBlock.trim(),
                    queryBlocks,
                    restBlocks.trim().substring(1, restBlocks.trim().length() - 1));
        }
        ...
        private String buildTagsInitialOptionsFromConfig() {
            return ticket.tags().stream()
                    .map(tag -> String.format("""
                        {"text": {"type": "plain_text", "text": "%s"}, "value": "%s"}""", findTagLabel(tag), tag))
                    .collect(joining(", "));
        }
        ...
        private String buildImpactsOptionsFromConfig() {
            return ticket.config().impacts().stream()
                    .map(imp -> String.format("""
                        {"text": {"type": "plain_text", "text": "%s"}, "value": "%s"}""", imp.label(), imp.code()))
                    .collect(joining(",", "[", "]"));
        }
        ...
        private String buildAssigneeBlock() {
            String options = config.mocks().slack().supportMembers().stream()
                    ...
                    {"text": {"type": "plain_text", "text": "%s"}, "value": "%s"}""", member.email(), member.userId()))
                    .collect(joining(",", "[", "]"));
