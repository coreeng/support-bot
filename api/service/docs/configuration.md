...
| `SUPPORT_BOT_VERSION` | Deployed Support Bot version reported to Elevate. The final API image defaults it from the build-time `P2P_VERSION`; a runtime environment value overrides that default. Local runs default to `dev`, while the Helm chart derives it from `image.tag` and then `Chart.appVersion` unless `supportBotVersion` is set explicitly. |
