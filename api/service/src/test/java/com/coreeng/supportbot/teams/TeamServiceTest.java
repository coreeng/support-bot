...
class TeamServiceTest {
    ...
    void listTeams_includesAllTeamTypes() {
        ...
        Team platformTeam = result.stream()
                .filter(t -> "platform1".equals(t.code()))
                .findFirst()
                .orElse(null);
        ...
        Team bothTeam = result.stream()
                .filter(t -> "both1".equals(t.code()))
                .findFirst()
                .orElse(null);
        ...
        Team supportTeam = result.stream()
                .filter(t -> "support".equals(t.code()))
                .findFirst()
                .orElse(null);
        ...
        long platformOnlyCount = result.stream()
                .filter(t -> t.types().equals(ImmutableList.of(TeamType.TENANT)))
                .count();
        long bothTypesCount = result.stream()
                .filter(t -> t.types().equals(ImmutableList.of(TeamType.TENANT, TeamType.ESCALATION)))
                .count();
        long supportOnlyCount = result.stream()
                .filter(t -> t.types().equals(ImmutableList.of(TeamType.SUPPORT)))
                .count();
    ...
    void listTeamsByUserEmail_returnsUserPlatformTeams() {
        ...
        when(platformTeamsService.listTeamsByUserEmail("other@test.com"))
                .thenReturn(ImmutableList.of(otherUserTeam1, otherUserTeam2));
