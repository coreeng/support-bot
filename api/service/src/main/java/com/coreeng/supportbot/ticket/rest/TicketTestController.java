...
public class TicketTestController {
    ...
    public TicketUI createTicket(@RequestBody TicketToCreate ticketToCreate) {
        Ticket ticket = repository.createTicketIfNotExists(
                Ticket.createNew(MessageTs.of(ticketToCreate.queryTs()), ticketToCreate.channelId()).toBuilder()
    ...
    public ResponseEntity<?> updateTicket(@RequestBody TicketToUpdate request) {
        ...
        if (!(result instanceof TicketSubmitResult.Success)) {
            return ResponseEntity.internalServerError().body("Expected a successful result, got: " + result);
        }
