...
public class HomepageViewMapper {
    ...
    public SlackView render(HomepageView homepage) {
        ...
        blocks.add(
                header(h -> h.text(plainText(":rocket: Admin Panel", true))),
                section(s -> s.text(markdownText(lastUpdatedMessage(homepage)))
                        .accessory(button(b ->
                                b.actionId(HomepageOperation.refresh.actionId()).text(plainText("Refresh"))))),
                divider());
        ...
        blocks.add(
                header(h -> h.text(plainText(":ticket: Support Query Tickets Summary", true))),
                section(s -> s.text(markdownText(viewDescriptionMessage(homepage)))
                        .accessory(button(b ->
                                b.actionId(HomepageOperation.filter.actionId()).text(plainText("Filter"))))),
                context(List.of(plainText("Total tickets: " + homepage.totalTickets()))),
                divider());
        blocks.addAll(homepage.tickets().stream()
                .flatMap(ticket -> renderTicket(ticket).stream())
                .collect(toImmutableList()));
        blocks.addAll(renderBottomBlock(homepage));
        return new SimpleSlackView(blocks.build(), privateMetadata(homepage));
    }
    ...
    private List<LayoutBlock> renderTicket(TicketView ticket) {
        ...
        List<LayoutBlock> blocks = new ArrayList<>(List.of(
                section(s -> s.text(markdownText(header)).accessory(button(b -> b.actionId(
                                TicketOperation.summaryView.actionId())
                        .text(plainText("Full Summary"))
                        .value(ticketSummaryViewMapper.createTriggerInput(new TicketSummaryViewInput(ticket.id())))))),
                section(s -> s.text(markdownText(format(
                        "*Impact*: %s",
                        ticket.impact() == null
                                ? "Not Evaluated"
                                : ticket.impact().label())))),
                section(s ->
                        s.text(markdownText(format("*Last Opened*: %s", formatSlackDate(ticket.lastOpenedAt())))))));
    ...
    private Optional<SectionBlock> buildTicketStatusSection(TicketView ticket) {
        return ticket.status() == TicketStatus.closed
                ? Optional.of(section(s -> s.fields(ImmutableList.of(
                        markdownText(format("*Closed*: %s", formatSlackDate(requireNonNull(ticket.closedAt()))))))))
                : Optional.empty();
    }
    ...
    private Optional<SectionBlock> buildEscalationSection(TicketView ticket) {
        return Optional.ofNullable(ticket.escalations())
                ...
                .map(escalations -> section(s -> {
                    ImmutableList<String> teams =
                            escalations.stream().map(Escalation::team).collect(toImmutableList());
                    return s.fields(
                            ImmutableList.of(markdownText(format("*Escalated to*: %s :rocket:", join(", ", teams)))));
                }));
    }

    private Optional<SectionBlock> buildInquiringTeamSection(TicketView ticket) {
        return Optional.ofNullable(ticket.inquiringTeam())
                .map(inquiringTeam -> section(
                        s -> s.fields(ImmutableList.of(markdownText(format("*Inquiring Team*: %s", inquiringTeam))))));
    }

    private Optional<SectionBlock> buildAssignedToSection(TicketView ticket) {
        return Optional.ofNullable(ticket.assignedTo())
                .map(assignedTo -> section(
                        s -> s.fields(ImmutableList.of(markdownText(format("*Assigned To*: %s", assignedTo))))));
    }
    ...
    private List<LayoutBlock> renderBottomBlock(HomepageView homepage) {
        ...
        if (homepage.page() > 0) {
            pageButtons.add(button(b -> b.actionId(HomepageOperation.previousPage.actionId())
                    .text(plainText("Previous Page"))
                    .value("previous")));
        }
        if (homepage.page() < homepage.totalPages() - 1) {
            pageButtons.add(button(b -> b.actionId(HomepageOperation.nextPage.actionId())
                    .text(plainText("Next Page"))
                    .value("next")));
        }
        ImmutableList.Builder<LayoutBlock> blocks = ImmutableList.builder();
        blocks.add(section(s -> s.text(plainText(pageNumberMessage(homepage)))));
