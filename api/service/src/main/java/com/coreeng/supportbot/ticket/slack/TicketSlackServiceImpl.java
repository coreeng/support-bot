...
public class TicketSlackServiceImpl implements TicketSlackService {
    ...
    public void markPostTracked(MessageRef threadRef) {
        addReactionToPostIfPresent(slackTicketsProps.responseInitialReaction(), threadRef);
    }

    @Override
    public void markTicketClosed(MessageRef threadRef) {
        addReactionToPostIfPresent(slackTicketsProps.resolvedReaction(), threadRef);
    }

    @Override
    public void markTicketEscalated(MessageRef threadRef) {
        addReactionToPostIfPresent(slackTicketsProps.escalatedReaction(), threadRef);
    }

    @Override
    public void unmarkTicketClosed(MessageRef threadRef) {
        removeReactionFromPostIfPresent("white_check_mark", threadRef);
    }
    ...
    public MessageRef postTicketForm(MessageRef threadRef, TicketCreatedMessage message) {
        ChatPostMessageResponse postMessageResponse = slackClient.postMessage(new SlackPostMessageRequest(
                createdMessageMapper.renderMessage(message), threadRef.channelId(), threadRef.ts()));
        MessageTs ts = MessageTs.of(postMessageResponse.getTs());
        log.info("Ticket form is posted: {}", ts);
        return new MessageRef(ts, threadRef.ts(), postMessageResponse.getChannel());
    }

    @Override
    public void editTicketForm(MessageRef threadRef, TicketCreatedMessage message) {
        if (threadRef.ts().mocked()) {
            log.atInfo().addArgument(threadRef::ts).log("Pretending to edit ticket form, because it's mocked: {}");
            return;
        }

        slackClient.editMessage(new SlackEditMessageRequest(
                createdMessageMapper.renderMessage(message), threadRef.channelId(), threadRef.ts()));
        log.atInfo().addArgument(threadRef::ts).log("Ticket form is updated: {}");
    }
    ...
    private void addReactionToPostIfPresent(String name, MessageRef messageRef) {
        ...
        try {
            ...
            log.atInfo().addArgument(name).addArgument(messageRef.ts()).log("Reaction({}) is posted to message({})");
        } catch (SlackException exc) {
            if (Objects.equals("already_reacted", exc.getError())) {
                log.atInfo().addArgument(messageRef.ts()).log("Reaction is already posted by bot to message({})");
            } else {
                throw exc;
            }
        }
    ...
    private void removeReactionFromPostIfPresent(String name, MessageRef messageRef) {
        if (messageRef.ts().mocked()) {
            log.atInfo()
                    .addArgument(name)
                    .addArgument(messageRef::ts)
        ...
        try {
            ...
            log.atInfo().addArgument(name).addArgument(messageRef.ts()).log("Reaction({}) is removed from message({})");
    ...
    public boolean isThreadReply(MessageRef messageRef) {
        ...
        try {
            ConversationsRepliesResponse response = slackClient.getThreadPage(ConversationsRepliesRequest.builder()
                    .channel(messageRef.channelId())
                    .ts(messageRef.ts().ts())
            ...
            return messages.stream()
                    .filter(m -> m.getTs() != null
                            && m.getTs().equals(messageRef.ts().ts()))
                    .findAny()
                    .map(m -> {
                        ...
                        boolean isReply =
                                m.getThreadTs() != null && !m.getThreadTs().equals(m.getTs());
                        if (isReply) {
                            log.atDebug().addKeyValue("ts", m.getTs()).log("Message is a thread reply");
                        } else {
                            log.atDebug().addKeyValue("ts", m.getTs()).log("Message is not a thread reply");
                        }
                        return isReply;
                    })
                    .orElse(false);
