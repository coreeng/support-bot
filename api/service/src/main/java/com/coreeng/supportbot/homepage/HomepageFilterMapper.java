...
public class HomepageFilterMapper {
    public static final String NO_TAGS_VALUE = "__no_tags__";
    public static final String NO_TAGS_LABEL = "-- No Tags --";
    ...
    public View.ViewBuilder render(HomepageView.State state, View.ViewBuilder view) {
        ...
        List<LayoutBlock> filterBlocks = new ArrayList<>(List.of(
                input(i -> i.label(plainText("Status")).optional(true).element(staticSelect(s -> s.actionId(
                                FilterField.status.actionId())
                        .initialOption(toOptionObjectOrNull(filter.status()))
                        .options(List.of(toOptionObject(TicketStatus.opened), toOptionObject(TicketStatus.closed)))))),
                input(i -> i.label(plainText("Escalation Team")).optional(true).element(staticSelect(s -> s.actionId(
                                FilterField.escalationTeam.actionId())
                        .initialOption(
                                filter.escalationTeam() == null
                                        ? null
                                        : toOptionObjectOrNull(escalationTeamsRegistry.findEscalationTeamByCode(
                                                filter.escalationTeam())))
                        .options(escalationTeamsRegistry.listAllEscalationTeams().stream()
                                .map(RenderingUtils::toOptionObject)
                                .toList())))),
                input(i -> i.label(plainText("Order")).optional(true).element(staticSelect(s -> s.actionId(
                                FilterField.order.actionId())
                        .initialOption(toOptionObjectOrNull(filter.order()))
                        .options(List.of(
                                toOptionObject(TicketsQuery.Order.desc), toOptionObject(TicketsQuery.Order.asc)))))),
                input(i -> i.label(plainText("Timeframe")).optional(true).element(staticSelect(s -> s.actionId(
                                FilterField.timeframe.actionId())
                ...
                input(i -> i.label(plainText("Impact")).optional(true).element(staticSelect(s -> s.actionId(
                        ...
                        .options(impactsRegistry.listAllImpacts().stream()
                                .map(RenderingUtils::toOptionObject)
                                .toList()))))));
        ...
        if (assignmentProps.enabled()) {
            ...
            filterBlocks.add(
                    input(i -> i.label(plainText("Assigned To")).optional(true).element(staticSelect(s -> s.actionId(
                                    FilterField.assignedTo.actionId())
                            .initialOption(
                                    filter.assignedTo() == null
                                            ? null
                                            : assigneeOptions.stream()
                                                    .filter(opt ->
                                                            opt.getValue().equals(filter.assignedTo()))
                                                    .findFirst()
                                                    .orElse(null))
                            .options(assigneeOptions)))));
        }

        return view.title(viewTitle(t -> t.type("plain_text").text("Tickets Filter")))
                .submit(viewSubmit(s -> s.type("plain_text").text("Apply Filters")))
    ...
    public HomepageFilter extractSubmittedValues(View view) {
        ...
        ImmutableMap<String, ViewState.Value> values = view.getState().getValues().entrySet().stream()
        ...
        if (orderValue != null && orderValue.getSelectedOption() != null) {
            builder.order(
                    TicketsQuery.Order.valueOf(orderValue.getSelectedOption().getValue()));
        }
        ...
        if (timeframeValue != null && timeframeValue.getSelectedOption() != null) {
            builder.timeframe(HomepageFilter.Timeframe.valueOf(
                    timeframeValue.getSelectedOption().getValue()));
        }
        ...
        if (tagsValue != null && tagsValue.getSelectedOptions() != null) {
            List<String> selectedValues = tagsValue.getSelectedOptions().stream()
                    ...
                    .toList();
            builder.includeNoTags(selectedValues.contains(NO_TAGS_VALUE));
            builder.tags(selectedValues.stream()
                    .filter(value -> !NO_TAGS_VALUE.equals(value))
                    .collect(toImmutableList()));
        }
