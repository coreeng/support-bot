...
public class SlackClientImpl implements SlackClient {
    ...
    public String getPermalink(SlackGetMessageByTsRequest request) {
        return Objects.requireNonNull(permalinkCache.get(request, () -> {
            if (request.ts().mocked()) {
                return "https://slack.com/" + request.channelId() + "/"
                        + request.ts().ts();
            }
    ...
    public User getUserById(SlackId.User userId) {
        return Objects.requireNonNull(userProfileCache.get(userId.id(), () -> doRequest(
                        "users.info",
                        () -> client.usersInfo(
                                UsersInfoRequest.builder().user(userId.id()).build()),
                        null)
                .getUser()));
    }
    ...
    @Override
    @Nullable public String getGroupName(SlackId.Group groupId) {
    ...
    @Override
    @Nullable public String getChannelName(String channelId) {
    ...
    private <V extends SlackApiTextResponse> V doRequest(
        ...
        try {
            ...
            if (!response.isOk()) {
                meterRegistry
                        .counter(
                                "slack_api_calls_errors_total",
                                "method",
                                methodName,
                                "error_code",
                                response.getError() != null ? response.getError() : "unknown")
                        .increment();
                throw new SlackException(
                        response,
                        errorMetadataExtractor != null ? errorMetadataExtractor.apply(response) : ImmutableList.of());
            }
            meterRegistry
                    .counter("slack_api_calls_success_total", "method", methodName)
                    .increment();
            return response;
