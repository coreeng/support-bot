...
public class TicketSummaryService {
        ...
        ImmutableList<TicketSummaryView.AssigneeOption> availableAssignees = assignmentProps.enabled()
                ? supportTeamService.members().stream()
                        .map(member -> new TicketSummaryView.AssigneeOption(
                                member.slackId().id(), member.email()))
    ...
    private TicketSummaryView.QuerySummaryView getQuerySummaryView(Ticket ticket, Message queryMessage) {
        ...
        return new TicketSummaryView.QuerySummaryView(blocks, ticket.queryTs(), senderId, permalink);
    }
    ...
    private TicketSummaryView.QuerySummaryView buildFallbackQuerySummary(Ticket ticket) {
        ...
        String permalink = resolveQueryPermalink(ticket);
        return new TicketSummaryView.QuerySummaryView(blocks, ticket.queryTs(), null, permalink);
    }
    ...
    private ImmutableList<LayoutBlock> buildBlocksForMessage(Message queryMessage) {
        ...
        if (isTombstone) {
            String tombstoneText = StringUtils.isBlank(text) ? "This message is unavailable." : text;
            return ImmutableList.of(section(s -> s.text(markdownText(t -> t.text(tombstoneText)))));
        }
        ...
        if (text != null && !text.isBlank()) {
            return ImmutableList.of(section(s -> s.text(markdownText(t -> t.text(text)))));
        }

        return ImmutableList.of(section(s -> s.text(markdownText(t -> t.text("No message content available.")))));
    }

    @Nullable private SlackId resolveSenderId(Message queryMessage) {
    ...
    @Nullable private String resolveQueryPermalink(Ticket ticket) {
        try {
            return slackClient.getPermalink(new SlackGetMessageByTsRequest(ticket.channelId(), ticket.queryTs()));
        } catch (SlackException ex) {
            log.atError()
                    .setCause(ex)
                    .addKeyValue("ticketId", ticket.id())
