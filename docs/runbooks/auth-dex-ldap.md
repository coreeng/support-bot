...
4. **Support Bot API** — deploy or upgrade the main app chart with `DEX_CLIENT_ID`, `DEX_CLIENT_SECRET`, `DEX_ISSUER_URI`, and application config for `platform-integration.jwt-groups` if you map LDAP groups to tenant teams. Dex is the only OAuth2 client the API registers; Google and Azure logins are reachable via Dex connectors (configured in your Dex deployment), not as separate front doors. The API **fails to start** if any of `DEX_CLIENT_ID` / `DEX_CLIENT_SECRET` / `DEX_ISSUER_URI` are unset, unless `security.test-bypass.enabled: true` (used by the functionaltests / integrationtests / nft profiles).
