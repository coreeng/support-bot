...
import { mockAuthorizationEndpoints, mockAuthorizationEndpointsError } from "../helpers/auth-mocks";
...
Given("the backend returns L2 support teams including {string}", async function (this: CustomWorld, teamName: string) {
  this.testContext = this.testContext || {};
  this.testContext.l2Teams = [{ label: teamName, code: teamName, types: ["escalation"] }];
});
...
Then("user should see {string} section", async function (this: CustomWorld, sectionText: string) {
  ...
  const selectedTeam =
    (
      await this.page
        .locator('[data-testid="team-selector-trigger"]')
        .textContent()
        .catch(() => null)
    )?.trim() ?? null;
  const pageContent = await this.page.content();
  const escalationsTeamsResp = await this.page.evaluate(() =>
    fetch("/team?type=escalation")
      .then((r) => r.json())
      .catch(() => null)
  );
...
Then("user should see {string} table", async function (this: CustomWorld, tableTitle: string) {
  ...
  const errorText = await this.page
    .getByText("Error loading escalations")
    .isVisible()
    .catch(() => false);
