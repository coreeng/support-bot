 ...
 * ```
 */
export async function mockAuthorizationEndpoints(page: Page, config: AuthMockConfig = {}): Promise<void> {
  const { leadershipEmails = [], supportEmails = [], l2Teams = [] } = config;
  ...
  await page.route(
    (url) => (url.pathname.includes("/team") || url.pathname.includes("/teams")) && /type=escalation/i.test(url.search),
    async (route: Route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
...
export async function mockAuthorizationEndpointsError(page: Page): Promise<void> {
  ...
  await page.route(
    (url) => url.pathname.includes("/team") && url.search.includes("type=escalation"),
    async (route: Route) => {
      await route.fulfill({ status: 500, body: "Internal Server Error" });
    }
  );
