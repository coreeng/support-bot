...
import { slackbotApiPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [slackbotApiPlugin],
    });

    await request(server).get('/api/slackbot-api/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/slackbot-api/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/slackbot-api/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/slackbot-api/todos/${createdTodoItem.id}`)
