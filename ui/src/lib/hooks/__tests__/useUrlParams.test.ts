...
import { enumValidator, isoDateValidator, nonNegativeIntValidator, useUrlParams } from "../useUrlParams";
...
describe("useUrlParams with validators", () => {
  ...
  it("returns the validated value when the raw URL value is valid", () => {
    ...
    const { result } = renderHook(() => useUrlParams(DEFAULTS_WITH_PAGE, VALIDATORS));
