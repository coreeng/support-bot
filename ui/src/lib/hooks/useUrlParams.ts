...
import { Registry } from "@/lib/hooks/index";
 ...
 */
export function enumValidator<T extends string>(validValues: readonly T[], fallback?: T): ParamValidator {
  return (raw: string, defaultValue: string): string =>
    (validValues as readonly string[]).includes(raw) ? raw : (fallback ?? defaultValue);
}
 ...
 */
export const nonNegativeIntValidator: ParamValidator = (raw: string, defaultValue: string): string => {
...
export function impactValidator(registry: Registry | undefined): ParamValidator {
  return (raw: string, defaultValue: string): string =>
    registry && registry.impacts.some((impact) => impact.code === raw) ? raw : defaultValue;
}
...
export function tagValidator(registry: Registry | undefined): ParamValidator {
  return (raw: string, defaultValue: string): string => (registry && registry.tags.some((tag) => tag.code === raw) ? raw : defaultValue);
}
 ...
 */
export const isoDateValidator: ParamValidator = (raw: string, defaultValue: string): string => {
 ...
 */
export function useUrlParams<T extends StringRecord>(defaults: T, validators?: ParamValidators<T>): [T, (updates: Partial<T>) => void] {
