import { DateFilter, formatTimeBucketLabel, getDateRangeFromFilter, PRESET_DAYS } from "../dateRange";
...
describe("getDateRangeFromFilter", () => {
  describe("named presets", () => {
    it("lastWeek returns the last 7 days", () => {
      expect(getDateRangeFromFilter({ ...BASE, dateFilter: "lastWeek" as DateFilter })).toEqual({ from: "2024-01-08", to: TODAY });
    });

    it("last2Weeks returns the last 14 days", () => {
      expect(getDateRangeFromFilter({ ...BASE, dateFilter: "last2Weeks" as DateFilter })).toEqual({ from: "2024-01-01", to: TODAY });
    });

    it("lastMonth returns the last 30 days", () => {
      expect(getDateRangeFromFilter({ ...BASE, dateFilter: "lastMonth" as DateFilter })).toEqual({ from: "2023-12-16", to: TODAY });
    });

    it("lastYear returns the last 365 days", () => {
      expect(getDateRangeFromFilter({ ...BASE, dateFilter: "lastYear" as DateFilter })).toEqual({ from: "2023-01-15", to: TODAY });
    });
  });
  ...
  describe("allValue", () => {
    it("returns { from: undefined, to: undefined } when dateFilter matches allValue", () => {
      expect(getDateRangeFromFilter({ ...BASE, dateFilter: "all" as DateFilter })).toEqual({ from: undefined, to: undefined });
    });
    ...
    it('does not treat "all" as allValue when allValue is not provided', () => {
      ...
      expect(getDateRangeFromFilter({ ...withoutAllValue, dateFilter: "all" as DateFilter })).toEqual({ from: undefined, to: undefined });
    });
  });

  describe("unknown filter", () => {
    it("returns { from: undefined, to: undefined } for a filter not in presetDays", () => {
      expect(getDateRangeFromFilter({ ...BASE, dateFilter: "bogus" as unknown as DateFilter })).toEqual({ from: undefined, to: undefined });
    });

    it("returns { from: undefined, to: undefined } for an empty string filter", () => {
      expect(getDateRangeFromFilter({ ...BASE, dateFilter: "" as unknown as DateFilter })).toEqual({ from: undefined, to: undefined });
    });
  });
