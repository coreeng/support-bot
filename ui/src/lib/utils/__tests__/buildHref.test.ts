...
describe('buildHref', () => {
    ...
    describe('valid params', () => {
        ...
        it('URL-encodes values with spaces', () => {
            expect(buildHref('/escalations', { team: 'Core Platform' }))
                .toBe('/escalations?team=Core+Platform')
        })

        it('URL-encodes values with special characters', () => {
            expect(buildHref('/tickets', { filter: 'a&b=c' }))
                .toBe('/tickets?filter=a%26b%3Dc')
        })
    })

    describe('mixed valid and empty/null/undefined values', () => {
        it('omits null values but keeps non-null ones', () => {
            expect(buildHref('/tickets', { team: 'platform', page: null }))
                .toBe('/tickets?team=platform')
        })

        it('omits undefined values but keeps non-undefined ones', () => {
            expect(buildHref('/tickets', { team: undefined, page: '3' }))
                .toBe('/tickets?page=3')
        })

        it('omits empty-string values but keeps non-empty ones', () => {
            expect(buildHref('/tickets', { team: 'platform', page: '' }))
                .toBe('/tickets?team=platform')
        })

        it('omits all three absent-value forms simultaneously', () => {
            expect(buildHref('/tickets', { team: 'eng', a: null, b: undefined, c: '' }))
                .toBe('/tickets?team=eng')
        })
    })

    describe('sidebar navigation use-case', () => {
        it('preserves the team param when navigating between pages', () => {
            expect(buildHref('/escalations', { team: 'platform' }))
                .toBe('/escalations?team=platform')
        })
