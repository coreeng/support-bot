...
export default function HealthPage() {
 ...
 <div>
 <h1 className="text-2xl font-bold text-foreground">Analytics &amp; Operations</h1>
 ...
 onValueChange={(v) => {
 setParams(v !== 'custom'
 ? { dateFilter: v, dateFrom: '', dateTo: '' }
 : { dateFilter: v })
 setCurrentPage(1)
 }}
 >
 <SelectTrigger className="w-[160px]" data-testid="health-date-filter"><SelectValue /></SelectTrigger>
 <SelectContent>
 ...
 onChange={e => { setParams({ dateFrom: e.target.value }); setCurrentPage(1) }}
 className="w-[150px]"/>
 <Input type="date" value={params.dateTo}
 onChange={e => { setParams({ dateTo: e.target.value }); setCurrentPage(1) }}
 className="w-[150px]"/>
 </>
 )}
 ...
 <h2 className="font-semibold mb-2 text-foreground text-center">Tickets Timeline</h2>
 ...
 <span className="font-mono text-2xl font-semibold tracking-tight tabular-nums text-warning">
 {avgRating.toFixed(1)}
 </span>
 <StarRating rating={avgRating} hideValue />
 <span className="text-xs text-muted-foreground">· {totalRatings} {totalRatings === 1 ? 'rating' : 'ratings'}</span>
 </div>
 </div>
 ...
 <h2 className="font-semibold mb-2 text-foreground text-center">Average Ticket Assignments per Support Engineer</h2>
 ...
 <div className="bg-card rounded-lg p-4 border mt-4">
 <h2 className="font-semibold mb-2 text-foreground text-center">Tickets Opened by Requesting Team (Top 10)</h2>
 <div style={{width: '100%', height: 300}}>
 <ResponsiveContainer width="100%" height="100%">
 ...
 <div className="bg-card rounded-lg p-4 border mt-4">
 <h2 className="font-semibold mb-2 text-foreground text-center">Current Active Tickets per Engineer</h2>
 <div style={{width: '100%', height: 300}}>
 <ResponsiveContainer width="100%" height="100%">
 ...
 <div className="bg-card rounded-lg p-4 border">
 <h2 className="font-semibold mb-2 text-foreground text-center">Tickets Opened by Hour of Day</h2>
 <p className="text-sm text-muted-foreground text-center mb-3">
 ...
 <div className="bg-card rounded-lg p-4 border">
 <h2 className="font-semibold mb-2 text-foreground text-center">Busiest Periods Heatmap</h2>
 <p className="text-xs text-muted-foreground text-center mb-3">Weekdays (Mon-Fri)</p>
 <div className="overflow-x-auto">
 <table className="w-full text-xs">
 <thead>
 <tr>
 <th className="text-left p-2 sticky left-0 bg-card z-10">Day</th>
 ...
 <td className="p-2 sticky left-0 bg-card font-medium">{row.day}</td>
 {Array.from({ length: 12 }, (_, idx) => {
 const hour = idx + 7 // Hours 7-18
 ...
 <div>
 <h2 className="text-base font-semibold text-foreground">Capacity vs Demand</h2>
 <p className="text-xs text-muted-foreground">Estimated engineer load for the selected window</p>
 </div>
 </div>
 ...
 <label htmlFor="engineers-on-rota" className="text-sm font-medium text-foreground">
 Engineers on Rota
 ...
 <div className="space-y-1.5">
 <label htmlFor="tickets-per-engineer" className="text-sm font-medium text-foreground">
 ...
 <span className="ml-1 text-xs font-normal text-muted-foreground">tickets</span>
 </p>
 </div>
 <div className="rounded-lg border bg-muted/40 p-3">
 ...
 <div className="rounded-lg border bg-muted/40 p-3">
 <p className="text-xs font-medium text-muted-foreground">Tickets per Engineer</p>
 <p className="mt-1 font-mono text-xl font-semibold tracking-tight tabular-nums text-foreground">
 ...
 capacityVsDemand.capacityUtilization > 80 ? 'text-destructive' :
 capacityVsDemand.capacityUtilization > 60 ? 'text-warning' :
 'text-success'
 }`}>
 {capacityVsDemand.capacityUtilization}%
 {capacityVsDemand.capacityUtilization > 100 && (
 <span className="ml-2 text-xs font-medium">Over capacity</span>
 )}
 </span>
 </div>
 ...
 capacityVsDemand.capacityUtilization > 80 ? 'bg-destructive' :
 capacityVsDemand.capacityUtilization > 60 ? 'bg-warning' :
 'bg-success'
 }`}
 ...
 <h3 className="font-semibold text-foreground text-sm">Capacity Insights by Time Block</h3>
 <span className="text-xs text-muted-foreground font-normal">
 ({capacityInsightsExpanded ? 'Click to collapse' : 'Click to expand'})
 </span>
 </div>
 <ChevronDown className={`w-4 h-4 text-foreground transition-transform duration-200 ${capacityInsightsExpanded ? 'rotate-180' : ''}`} />
 </button>
 ...
 <div className="w-3 h-3 bg-destructive/10 border border-destructive/30 rounded"></div>
 <span>Over Capacity (&gt;100%)</span>
 </div>
 <div className="flex items-center gap-1">
 <div className="w-3 h-3 bg-warning/10 border border-warning/30 rounded"></div>
 <span>Near Capacity (80-100%)</span>
 </div>
 <div className="flex items-center gap-1">
 <div className="w-3 h-3 bg-success/10 border border-success/30 rounded"></div>
 ...
 <div className="rounded-xl border bg-card p-5">
 <p className="text-sm font-medium text-muted-foreground">Consolidated Average Rating</p>
 <div className="mt-2 flex items-baseline gap-2">
 <span className="font-mono text-2xl font-semibold tracking-tight tabular-nums text-warning">
 {avgRating.toFixed(1)}
 </span>
 ...
 <div className="space-y-6">
 <div className="bg-card rounded-lg p-4 border">
 <h2 className="text-xl font-semibold mb-2 text-foreground text-center">Ratings Received per Week</h2>
 ...
 <div className="bg-card rounded-lg p-4 border">
 <h2 className="text-xl font-semibold mb-2 text-foreground text-center">Average Rating per Week</h2>
 <div style={{width: '100%', height: 320}}>
 <ResponsiveContainer width="100%" height="100%">
 ...
 <XAxis dataKey="weekStart" tickFormatter={(d: string) => new Date(d).toLocaleDateString()}
 stroke="var(--border)" tick={{ fill: 'var(--muted-foreground)', fontSize: 11 }} />
 <YAxis domain={[0, 5]} tickCount={6}  stroke="var(--border)" tick={{ fill: 'var(--muted-foreground)', fontSize: 11 }}/>
 <Tooltip labelFormatter={(label) => new Date(label as string).toLocaleDateString()}
 contentStyle={{ background: 'var(--popover)', color: 'var(--popover-foreground)', border: '1px solid var(--border)', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }} labelStyle={{ color: 'var(--popover-foreground)' }} itemStyle={{ color: 'var(--popover-foreground)' }} cursor={{ stroke: 'var(--border)', fill: 'var(--accent)' }} />
 <Legend />
 ...
 <h3 className="text-base font-semibold text-foreground">Bulk Reassign Tickets</h3>
 </div>
 <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform duration-[200ms] ${bulkReassignExpanded ? '' : '-rotate-90'}`} />
 </button>

 {bulkReassignExpanded && (
 <div className="px-6 pb-6 pt-2 space-y-3 animate-in fade-in slide-in-from-top-1 duration-[200ms]">
 <div className="flex flex-wrap items-end gap-3">
 <div className="flex flex-col gap-1.5">
 <Label htmlFor="bulk-from" className="text-sm font-medium text-foreground">From</Label>
 ...
 <div className="flex flex-col gap-1.5">
 <Label htmlFor="bulk-to" className="text-sm font-medium text-foreground">To</Label>
 ...
 ).length} open ticket{filteredTickets.filter(t =>
 t.status?.toLowerCase() === 'opened' && (
 bulkReassignFrom === 'unassigned'
 ? !t.assignedTo
 : t.assignedTo === bulkReassignFrom
 ...
 Only open tickets will be reassigned.
 </p>
 ...
 <div className="mt-3 bg-warning/10 border border-warning/30 rounded-md p-3">
 <div className="flex items-start gap-2">
 <AlertTriangle className="w-4 h-4 text-warning flex-shrink-0 mt-0.5" />
 <div className="flex-1">
 <h4 className="font-semibold text-foreground text-sm mb-1">Confirm Bulk Reassignment</h4>
 ...
 className="px-3 py-1.5 text-xs bg-warning text-white rounded hover:bg-warning disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium flex items-center gap-1.5"
 >
 {isReassigning ? (
 <>
 ...
 disabled={isReassigning}
 className="px-3 py-1.5 text-xs bg-muted text-foreground rounded hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium"
 >
 Cancel
 </button>
 </div>
 ...
 onChange={(v) => { setInquiringTeamFilter(v ?? ''); setCurrentPage(1) }}
 options={inquiringTeamsWithTickets.map(team => ({ label: team, value: team }))}
 />
 <SingleSelectFilter
 title="Escalated To"
 value={escalatedTeamFilter || undefined}
 onChange={(v) => { setEscalatedTeamFilter(v ?? ''); setCurrentPage(1) }}
 ...
 ...((supportMembers ?? []).map((member: SupportMember) => ({
 label: member.displayName, value: member.displayName,
 }))),
 ]}
 />
 )}
 ...
 onChange={(v) => { setRatedFilter(v ?? ''); setCurrentPage(1) }}
 showSearch={false}
 options={[
 { label: 'Yes', value: 'yes' },
 { label: 'No', value: 'no' },
 ...
 <TableRow>
 <TableCell colSpan={isAssignmentEnabled ? 7 : 6} className="text-center text-muted-foreground py-8">
 ...
 ? (t.escalations || []).map((e: Escalation) => e.team?.name).filter(Boolean).join(', ')
 : <span className="text-muted-foreground">None</span>}
 </TableCell>
 <TableCell>
 <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${statusColors[t.status?.toLowerCase()] || 'bg-muted text-foreground'}`}>
 {t.status}
 </span>
 </TableCell>
...
export function MetricCard({title, value, extra, color}: {
 ...
 <div className={`mt-2 font-mono text-2xl font-semibold tracking-tight tabular-nums ${color ? valueColor[color] ?? 'text-foreground' : 'text-foreground'}`}>{value}</div>
...
function StarRating({rating, hideValue = false}: { rating: number; hideValue?: boolean }) {
 ...
 return <Star key={i} className="h-4 w-4 text-warning" />
 }
