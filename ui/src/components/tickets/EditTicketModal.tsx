...
import { useAssignmentEnabled, useRegistry, useSupportMembers, useTeamSuggestions, useTenantTeams, useTicket } from "@/lib/hooks";
import { Escalation, SupportMember, TicketImpact, TicketTag, TicketWithLogs } from "@/lib/types";
import { AlertCircle, Slack, Ticket } from "lucide-react";
...
export default function EditTicketModal({ ticketId, open, onOpenChange, onSuccess }: EditTicketModalProps) {
  ...
  const { data: teamSuggestionsData, isError: isTeamSuggestionsError } = useTeamSuggestions(ticketId ? Number(ticketId) : undefined);
  ...
  useEffect(() => {
    ...
    const hasUnresolvedEscalations = ticketDetails.escalations?.some((esc: Escalation) => !esc.resolvedAt) || false;
  ...
  const handleTagToggle = (tagCode: string) => {
    setSelectedTags((prev) => (prev.includes(tagCode) ? prev.filter((t) => t !== tagCode) : [...prev, tagCode]));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const assignedUserId =
        isAssignmentEnabled && assignedTo ? supportMembers?.find((m) => m.displayName === assignedTo)?.userId || null : null;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent data-testid="edit-ticket-modal" className="max-h-[90vh] max-w-3xl min-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl">
            <Ticket className="text-primary h-5 w-5" />
            Ticket #{ticketId}
            {!canEdit && <span className="text-muted-foreground ml-1 text-xs font-normal">(Read-only)</span>}
          </DialogTitle>
          <DialogDescription className="text-sm">
            {canEdit ? "Edit ticket details below." : "View ticket details. Only support engineers can edit tickets."}
          </DialogDescription>
        </DialogHeader>

        {ticketError && (
          <div className="bg-destructive/10 border-destructive/30 text-destructive rounded-lg border-2 p-4 text-sm font-medium">
          ...
          <div className="space-y-4 pt-2">
            ...
            {displayTicket?.query?.text && (
              <div className="space-y-1.5">
                <Label className="text-foreground text-sm font-medium">Ticket Message</Label>
                <div className="bg-muted/40 rounded-md border px-3 py-2">
                  <SlackMessageRenderer text={displayTicket.query.text} className="text-sm break-words" />
                  {displayTicket.query.date && (
                    <p className="text-muted-foreground mt-2 text-xs">{new Date(displayTicket.query.date).toLocaleString()}</p>
                  )}
                </div>
              </div>
            )}
            ...
            {displayTicket?.summary?.trim() && (
              <div className="space-y-1.5">
                <Label className="text-foreground text-sm font-medium">AI Summary</Label>
                <div className="bg-muted/40 rounded-md border px-3 py-2">
                  <p className="text-foreground text-sm leading-6 break-words whitespace-pre-wrap">{displayTicket.summary}</p>
                </div>
              </div>
            )}
            ...
            {/* Status History */}
            <div className="space-y-1.5">
              <Label className="text-foreground text-sm font-medium">Status History</Label>
              <div className="bg-muted/40 max-h-40 overflow-y-auto rounded-md border text-sm">
                <ul className="divide-y">
                  {displayTicket.logs?.map((log: { event: string; date: string }, idx: number) => (
                    <li key={idx} className="hover:bg-muted/60 flex h-9 items-center justify-between px-3 transition">
                      <span className="text-foreground font-medium">{log.event}</span>
                      <span className="text-muted-foreground text-xs">{new Date(log.date).toLocaleString()}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {/* Escalations */}
            <div className="space-y-1.5">
              <Label className="text-foreground text-sm font-medium">Escalations</Label>
              <div className="bg-muted/40 rounded-md border text-sm">
                {displayTicket.escalations?.length ? (
                  ...
                  <div className="text-muted-foreground flex h-9 items-center px-3">No escalations</div>
                )}
              </div>
            </div>
            ...
            {/* Change Status */}
            <div className="space-y-1.5">
              <Label htmlFor="status-select" className="text-foreground text-sm font-medium">
              ...
              {canEdit ? (
                <div className="space-y-1">
                  ...
                  {validationErrors.status && <p className="text-destructive text-sm">{validationErrors.status}</p>}
                      ...
                      <div className="flex gap-2">
                        ...
                        <div className="flex-1">
                          <p className="text-warning text-sm font-semibold">
                            Ticket has {ticketDetails?.escalations?.filter((esc: Escalation) => !esc.resolvedAt).length} unresolved
                            escalation
                            {(ticketDetails?.escalations?.filter((esc: Escalation) => !esc.resolvedAt).length || 0) > 1 ? "s" : ""}.
                          </p>
                          <p className="text-warning mt-1 text-sm">Closing the ticket will close all related escalations.</p>
                        </div>
                      </div>
                    </div>
                  )}
            ...
            {isAssignmentEnabled && (
              <div className="space-y-1.5">
                <Label htmlFor="assignee-select" className="text-foreground text-sm font-medium">
                  Support Engineer
                </Label>
                {canEdit ? (
                  <Select value={assignedTo || "__unassigned"} onValueChange={(v) => setAssignedTo(v === "__unassigned" ? "" : v)}>
                  ...
                  <p className="text-foreground text-sm">{assignedTo || "Unassigned"}</p>
                )}
              </div>
            )}
            ...
            {/* Author's Team */}
            <div className="space-y-1.5">
              <Label htmlFor="team-select" className="text-foreground text-sm font-medium">
                ...
                (() => {
                  ...
                  return (
                    <div className="space-y-1">
                      ...
                      >
                        <SelectTrigger id="team-select" className={`w-full ${validationErrors.authorTeam ? "border-destructive" : ""}`}>
                      ...
                      {validationErrors.authorTeam && <p className="text-destructive text-sm">{validationErrors.authorTeam}</p>}
                    </div>
                  );
                })()
              ) : (
            ...
            {/* Tags */}
            <div className="space-y-1.5">
              <Label htmlFor="tags-select" className="text-foreground text-sm font-medium">
              ...
              {canEdit ? (
                <div className="space-y-1">
                  ...
                  />
                  {validationErrors.tags && <p className="text-destructive text-sm">{validationErrors.tags}</p>}
                </div>
              ) : (
                <div className="flex flex-wrap gap-1.5">
                  {(displayTicket.tags ?? []).length ? (
                    (displayTicket.tags as string[]).map((tag: string, idx: number) => (
                      <span key={idx} className="text-foreground rounded-full border px-2 py-0.5 text-xs">
                        {tag}
                      </span>
                    ))
                  ) : (
            ...
            {/* Impact */}
            <div className="space-y-1.5">
              <Label htmlFor="impact-select" className="text-foreground text-sm font-medium">
          ...
          <div className="flex-1">
            {displayTicket?.query?.link && (
              <Button asChild variant="outline" size="sm">
                <a href={displayTicket.query.link} target="_blank" rel="noopener noreferrer">
          ...
          <div className="flex gap-2">
            {canEdit && (
              <>
                ...
                <Button onClick={handleSave} disabled={isSaving || !status || !impact || !authorTeam || selectedTags.length === 0}>
                  {isSaving ? "Saving..." : "Save Changes"}
                </Button>
              </>
            )}
            {!canEdit && <Button onClick={() => onOpenChange(false)}>Close</Button>}
          </div>
        </DialogFooter>
      </DialogContent>
    </Dialog>
