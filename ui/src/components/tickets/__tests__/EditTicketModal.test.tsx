...
const mockTicketDetails = {
  ...
  impact: "high",
  tags: ["bug", "urgent"],
  escalations: [{ id: "esc1", team: { name: "Support" } }],
  logs: [
...
const mockTeams = [{ name: "Engineering" }, { name: "Support" }, { name: "Product" }];
...
describe("EditTicketModal", () => {
  ...
  describe("Modal Visibility", () => {
    it("renders modal when open is true", () => {
      render(<EditTicketModal ticketId="123" open={true} onOpenChange={mockOnOpenChange} onSuccess={mockOnSuccess} />, {
        wrapper: Wrapper,
      });
    ...
    it("does not render modal when open is false", () => {
      render(<EditTicketModal ticketId="123" open={false} onOpenChange={mockOnOpenChange} onSuccess={mockOnSuccess} />, {
        wrapper: Wrapper,
      });
    ...
    it("does not show warning when changing to closed with only resolved escalations", () => {
      mockUseTicket.mockReturnValue({
        data: {
          ...mockTicketDetails,
          status: "opened",
          escalations: [{ id: "esc1", team: { name: "Support" }, resolvedAt: "2024-01-01T10:00:00Z" }],
        },
        isLoading: false,
        error: null,
      } as unknown as ReturnType<typeof hooks.useTicket>);
    ...
    it("does not show warning when ticket is already closed", () => {
      mockUseTicket.mockReturnValue({
        data: {
          ...mockTicketDetails,
          status: "closed",
          escalations: [{ id: "esc1", team: { name: "Support" }, resolvedAt: null }],
        },
        isLoading: false,
        error: null,
      } as unknown as ReturnType<typeof hooks.useTicket>);
