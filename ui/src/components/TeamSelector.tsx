...
export default function TeamSelector() {
    ...
    return (
      <div className="group relative">
        <button
          ...
          className="border-warning/30 bg-warning/10 text-warning hover:bg-warning/20 focus-visible:ring-ring flex h-9 w-9 items-center justify-center rounded-md border transition-colors focus-visible:ring-2 focus-visible:outline-none"
        >
          <AlertTriangle className="h-4 w-4" />
        </button>
        <div
          role="tooltip"
          className="border-warning/30 bg-warning/10 text-warning invisible absolute top-full right-0 z-50 mt-2 w-72 rounded-md border px-3 py-2 opacity-0 shadow-md transition-opacity group-focus-within:visible group-focus-within:opacity-100 group-hover:visible group-hover:opacity-100"
        >
  ...
  return (
    <>
      <Dialog
        open={deniedTeam !== null}
        onOpenChange={(open) => {
          if (!open) setDeniedTeam(null);
        }}
      >
        <DialogContent data-testid="team-access-denied-modal">
          <DialogHeader>
            ...
            <DialogDescription>
              The link you followed requests the team <strong>{deniedTeam}</strong>, which you don&apos;t have access to.
              {fallbackTeam && (
                <>
                  {" "}
