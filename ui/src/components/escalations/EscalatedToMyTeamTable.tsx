  ...
  return (
    <div className="bg-card space-y-4 rounded-xl border p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-foreground text-base font-semibold">Escalated to My Team</h2>
          <p className="text-muted-foreground mt-1 text-xs">Escalations routed to your team for handling</p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <SingleSelectFilter
          ...
          onChange={(v) => {
            setStatusFilter((v ?? "all") as "all" | "ongoing" | "resolved");
            setPageIndex(0);
          }}
          showSearch={false}
          options={[
      ...
      {topTags.length > 0 && (
        <div>
          <h3 className="text-muted-foreground mb-2 text-xs font-medium tracking-wide uppercase">Top 5 Tags</h3>
      ...
      {filteredEscalations.length === 0 ? (
        <p className="text-muted-foreground py-8 text-center">No escalations found for your team.</p>
      ) : (
        <>
                ...
                <tr>
                  {(() => {
                    const SortableHeader = ({ col, label }: { col: SortColumn; label: string }) => (
                      <th
                        className="text-foreground hover:bg-muted/70 cursor-pointer px-4 py-2 text-left text-xs font-bold uppercase transition-colors select-none"
          ...
          {filteredEscalations.length > pageSize && (
            <div className="flex items-center justify-between gap-4">
              <Button variant="outline" onClick={() => setPageIndex((prev) => Math.max(0, prev - 1))} disabled={pageIndex === 0}>
                Previous
              </Button>
