...
import { ArrowUp, ArrowDown, ArrowUpDown, Search, ExternalLink, HelpCircle } from 'lucide-react'
...
export default function InFlightPrsTab() {
 ...
 <div>
 <h2 className="text-base font-semibold text-foreground">In-Flight PRs</h2>
 <p className="text-sm text-muted-foreground">Currently open pull requests across tracked repositories</p>
 </div>
 <Select
 value={teamFilter || ALL_TEAMS}
 onValueChange={(v) => { setTeamFilter(v === ALL_TEAMS ? '' : v); setPage(0) }}
 >
 ...
 <SortHeader label="Last Review" sortKey="lastReview" align="right" activeSortKey={sortKey} sortDir={sortDir} onSort={handleSort} />
 <SortHeader label="Team" sortKey="team" align="left" activeSortKey={sortKey} sortDir={sortDir} onSort={handleSort} />
 <th className="px-4 py-2 text-right text-xs font-bold text-foreground uppercase">Slack</th>
 </tr>
 </thead>
 ...
 <td className="px-4 py-2 text-sm text-muted-foreground">{pr.waitingOn}</td>
 ...
 <td className="px-4 py-2 text-right tabular-nums font-mono text-sm text-muted-foreground">{age.label}</td>
 <td className="px-4 py-2 text-right text-sm text-muted-foreground">{relativeTime(pr.lastReviewAt)}</td>
 <td className="px-4 py-2 text-sm text-muted-foreground">{pr.owningTeamLabel}</td>
 <td className="px-4 py-2 text-right">
...
function StatCard({ label, value, isLoading, valueClass, accent = 'primary' }: {
 label: string
 value: number
 isLoading: boolean
 valueClass?: string
...
function SortHeader({ label, sortKey, activeSortKey, sortDir, onSort, align }: {
 label: string
 sortKey: SortKey
