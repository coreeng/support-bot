...
export const SlackMessageRenderer: React.FC<SlackMessageRendererProps> = ({ text, className = "" }) => {
      ...
      if (segment.type === "codeblock") {
        ...
        elements.push(
          <pre
            ...
            className="bg-muted text-foreground my-2 overflow-x-auto rounded-md border p-3 font-mono text-sm"
          >
  ...
  const parseInlineFormatting = (text: string, baseKey: number): React.ReactNode[] => {
    ...
    while ((match = inlineCodeRegex.exec(text)) !== null) {
      ...
      // Add inline code
      elements.push(
        <code key={`inline-${keyCounter++}`} className="bg-muted text-foreground rounded border px-1.5 py-0.5 font-mono text-sm">
          {match[1]}
        </code>
      );
      lastIndex = match.index + match[0].length;
  ...
  return <div className={`text-foreground leading-relaxed ${className}`}>{renderedContent}</div>;
};
