...
export default function DashboardsPage() {
  ...
  const {
    data: firstResponsePercentiles,
    refetch: refetchFirstResponse,
    isFetching: isFetchingFirstResponse,
  } = useFirstResponsePercentiles(activeSection === "response" && isDateRangeValid, dateRange.from, dateRange.to);
  const {
    data: durationDistribution,
    isLoading: isDistributionLoading,
    refetch: refetchDistribution,
    isFetching: isFetchingDistribution,
  } = useFirstResponseDurationDistribution(activeSection === "response" && isDateRangeValid, dateRange.from, dateRange.to);
  const {
    data: unattendedQueries,
    isLoading: isUnattendedLoading,
    refetch: refetchUnattended,
    isFetching: isFetchingUnattended,
  } = useUnattendedQueriesCount(activeSection === "response" && isDateRangeValid, dateRange.from, dateRange.to);
  ...
  const {
    data: resolutionPercentiles,
    refetch: refetchResolutionPercentiles,
    isFetching: isFetchingResolutionPerc,
  } = useTicketResolutionPercentiles(activeSection === "resolution" && isDateRangeValid, dateRange.from, dateRange.to);
  const {
    data: resolutionDurationDistribution,
    isLoading: isResolutionDistributionLoading,
    refetch: refetchResolutionDistribution,
    isFetching: isFetchingResDist,
  } = useTicketResolutionDurationDistribution(activeSection === "resolution" && isDateRangeValid, dateRange.from, dateRange.to) as {
  ...
  const {
    data: resolutionTimesByWeek,
    refetch: refetchResolutionByWeek,
    isFetching: isFetchingResByWeek,
  } = useResolutionTimesByWeek(activeSection === "resolution" && isDateRangeValid, dateRange.from, dateRange.to);
  const {
    data: unresolvedTicketAges,
    refetch: refetchUnresolvedAges,
    isFetching: isFetchingUnresolvedAges,
  } = useUnresolvedTicketAges(activeSection === "resolution" && isDateRangeValid, dateRange.from, dateRange.to);
  const {
    data: incomingVsResolvedRate,
    refetch: refetchIncomingVsResolved,
    isFetching: isFetchingIncomingVsResolved,
  } = useIncomingVsResolvedRate(activeSection === "resolution" && isDateRangeValid, dateRange.from, dateRange.to, { granularity: "AUTO" });

  const {
    data: avgEscalationDurationByTag,
    refetch: refetchAvgEscDuration,
    isFetching: isFetchingAvgEsc,
  } = useAvgEscalationDurationByTag(activeSection === "escalation" && isDateRangeValid, dateRange.from, dateRange.to);
  const {
    data: escalationPercentageByTag,
    refetch: refetchEscPercentage,
    isFetching: isFetchingEscPerc,
  } = useEscalationPercentageByTag(activeSection === "escalation" && isDateRangeValid, dateRange.from, dateRange.to);
  const {
    data: escalationTrendsByDate,
    refetch: refetchEscTrends,
    isFetching: isFetchingEscTrends,
  } = useEscalationTrendsByDate(activeSection === "escalation" && isDateRangeValid, dateRange.from, dateRange.to);
  const {
    data: escalationsByTeam,
    refetch: refetchEscByTeam,
    isFetching: isFetchingEscTeam,
  } = useEscalationsByTeam(activeSection === "escalation" && isDateRangeValid, dateRange.from, dateRange.to);
  const {
    data: escalationsByImpact,
    refetch: refetchEscByImpact,
    isFetching: isFetchingEscImpact,
  } = useEscalationsByImpact(activeSection === "escalation" && isDateRangeValid, dateRange.from, dateRange.to);
  const isRefreshingEscalationSla =
    isFetchingAvgEsc || isFetchingEscPerc || isFetchingEscTrends || isFetchingEscTeam || isFetchingEscImpact;

  // Weekly SLA Trends - only load when section is active
  const {
    data: weeklyTicketCounts,
    refetch: refetchWeeklyCounts,
    isFetching: isFetchingWeeklyCounts,
  } = useWeeklyTicketCounts(activeSection === "weekly");
  const {
    data: weeklyComparison,
    refetch: refetchWeeklyComparison,
    isFetching: isFetchingWeeklyComp,
  } = useWeeklyComparison(activeSection === "weekly");
  const {
    data: topEscalatedTags,
    refetch: refetchTopEscTags,
    isFetching: isFetchingTopEsc,
  } = useTopEscalatedTagsThisWeek(activeSection === "weekly");
  const {
    data: resolutionTimeByTag,
    refetch: refetchResTimeByTag,
    isFetching: isFetchingResTimeByTag,
  } = useResolutionTimeByTag(activeSection === "resolution" && isDateRangeValid, dateRange.from, dateRange.to);

  const isRefreshingResolutionSla =
    isFetchingResolutionPerc ||
    isFetchingResDist ||
    isFetchingResByWeek ||
    isFetchingUnresolvedAges ||
    isFetchingResTimeByTag ||
    isFetchingIncomingVsResolved;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-foreground text-2xl font-bold">SLA Dashboards</h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger className="w-[160px]" data-testid="sla-date-filter">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
