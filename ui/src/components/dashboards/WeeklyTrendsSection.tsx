...
export function WeeklyTrendsSection({
 ...
 <div>
 <h2 className="text-base font-semibold text-foreground">Weekly Trends</h2>
 <p className="text-sm text-muted-foreground">Monitor weekly metrics and patterns</p>
 </div>
 ...
 return (
 <div key={metric.label} className="bg-card rounded-xl p-6 border">
 <h3 className="text-base font-semibold text-foreground capitalize mb-2">
 ...
 <p className="text-xs text-muted-foreground mt-1">Last week: {metric.lastWeek}</p>
 </div>
 )
 })}
 </div>
 ...
 stroke="var(--border)"/>
 <YAxis label={{ value: 'Tickets (Avg)', angle: -90, position: 'insideLeft' }} stroke="var(--border)" tick={{ fill: 'var(--muted-foreground)', fontSize: 11 }}/>
 <Tooltip contentStyle={{ background: 'var(--popover)', color: 'var(--popover-foreground)', border: '1px solid var(--border)', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }} labelStyle={{ color: 'var(--popover-foreground)' }} itemStyle={{ color: 'var(--popover-foreground)' }} cursor={{ stroke: 'var(--border)', fill: 'var(--accent)' }} />
 <Legend 
