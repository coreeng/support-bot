...
export const TeamFilterProvider = ({ children }: { children: ReactNode }) => {
  ...
  const teamScope = useMemo<TeamScopeState>(() => {
    ...
    if (selectedTeam === null) {
      const firstDataTeam = user.teams.find((t) => !(t.types || []).some((type) => /leadership/i.test(type) || /support/i.test(type)));
      return firstDataTeam ? { mode: "selected_teams", teams: [firstDataTeam.name] } : { mode: "all_teams" };
    }
  ...
  const allTeams = useMemo(() => {
    if (!user) return [];
    return user.teams.filter((t) => !(t.types || []).some((type) => /leadership/i.test(type) || /support/i.test(type))).map((t) => t.name);
  }, [user]);
