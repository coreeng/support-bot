...
describe("LoginPage", () => {
  ...
  describe("popup flow (window.opener present)", () => {
    ...
    it("does NOT send postMessage or close on signIn error", async () => {
      ...
      await waitFor(() => {
        expect(mockReplace).toHaveBeenCalledWith(expect.stringContaining("/login?error="));
      });
    ...
    it("sanitizes callbackUrl sent in postMessage", async () => {
      ...
      await waitFor(() => {
        ...
        expect(mockPostMessage).toHaveBeenCalledWith({ type: "auth:success", callbackUrl: "/" }, window.location.origin);
      });
    });
  ...
  describe("non-popup flow", () => {
    ...
    it("sanitizes callbackUrl in authenticated redirect", async () => {
      mockUseAuth.mockReturnValue({ isLoading: false, isAuthenticated: true } as any);
      mockUseSearchParams.mockReturnValue(new URLSearchParams("callbackUrl=https://evil.com") as any);
