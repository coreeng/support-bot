...
export async function GET() {
  try {
    ...
    if (!response.ok) {
      console.error(`[IdentityProviders] Backend returned HTTP ${response.status}.`);
      return NextResponse.json({ providers: [], error: true }, { headers: NO_CACHE_HEADERS });
    }

    const body = await response.json();
    return NextResponse.json(body, { headers: NO_CACHE_HEADERS });
  } catch (error) {
    console.error("[IdentityProviders] Cannot contact backend. Error:", error);
    return NextResponse.json({ providers: [], error: true }, { headers: NO_CACHE_HEADERS });
  }
}
