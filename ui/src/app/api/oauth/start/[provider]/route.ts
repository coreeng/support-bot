...
export async function GET(request: NextRequest, { params }: { params: Promise<{ provider: string }> }) {
  ...
  if (isOauthUiKnownProvider(provider)) {
    const resolved = tryResolvePublicOrigin(request.nextUrl.origin, sanitizeCallbackUrl(request.nextUrl.searchParams.get("callbackUrl")));
    if (!resolved.ok) {
      return resolved.response;
    }
    const { origin } = resolved;
    const redirectUri = new URL(`/api/oauth/callback/${provider}`, origin).toString();
    ...
    if (oauthUrl.ok) {
      try {
        ...
        const userCallbackUrl = sanitizeCallbackUrl(request.nextUrl.searchParams.get("callbackUrl"));
