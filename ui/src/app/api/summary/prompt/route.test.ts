...
// useSummaryPrompt parses, including the forwarded backend `code`.
...
describe("summary prompt BFF route", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("forwards to the backend prompt endpoint and returns its payload", async () => {
    ...
    expect(mockBackendFetch).toHaveBeenCalledWith(request, "/summary/prompt");
